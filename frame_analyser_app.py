...
MODELS = {
    ...
    "Qwen Portal — vision-model (free, requires re-auth every 4h)": {
        "type": "qwen-portal",
        "id": "vision-model"
    },
...
def get_qwen_token() -> str | None:
    """Read current Qwen OAuth token from OpenClaw auth profiles."""
    try:
        profiles = json.loads(AUTH_PROFILES_PATH.read_text())
        token = profiles.get("profiles", {}).get("qwen-portal:default", {}).get("access")
        expires = profiles.get("profiles", {}).get("qwen-portal:default", {}).get("expires", 0)
...
def analyse_frame_qwen(image_path: Path, full_prompt: str, timeout: int) -> dict:
    """Analyse frame using Qwen Portal vision model via OpenAI-compatible API."""
    token = get_qwen_token()
    if not token:
        raise Exception("Qwen token expired — run: openclaw models auth login --provider qwen-portal")
...
def parse_scores(raw: str) -> dict:
    ...
    if start >= 0 and end > start:
        ...
        return {
            "composition": min(10, max(0, int(scores.get("composition", 0)))),
            "light_and_colour": min(10, max(0, int(scores.get("light_and_colour", 0)))),
            "emotional_resonance": min(10, max(0, int(scores.get("emotional_resonance", 0)))),
            "narrative": min(10, max(0, int(scores.get("narrative", 0)))),
...
def generate_report(top: list[dict], video_title: str, interval: int, model_label: str) -> str:
    ...
    lines += [f"---", f"", f"*Generated by Frame Analyser 🎬*"]
    return "\n".join(lines)


# ── UI ──────────────────────────────────────────────────────────────────────────

st.set_page_config(page_title="Frame Analyser", page_icon="🎬", layout="centered")
...
video_url = st.text_input("Video URL", placeholder="Paste a YouTube link — e.g. https://youtu.be/xxxxx")
...
# Show token status for Qwen Portal
if model_config["type"] == "qwen-portal":
    token = get_qwen_token()
    if token:
        st.success("Qwen Portal token valid")
    else:
        st.error("Qwen token expired — run: openclaw models auth login --provider qwen-portal")

# API key input for cloud models
api_key = None
if model_config["type"] in ["gemini", "anthropic"]:
...
if st.button("🎬 Analyse", type="primary", use_container_width=True):
    ...
    if model_config["type"] == "qwen-portal" and not get_qwen_token():
        st.error("Qwen token expired — run: openclaw models auth login --provider qwen-portal")
        st.stop()

    if model_config["type"] in ["gemini", "anthropic"] and not api_key:
    ...
    try:
        with st.status("Getting video info...") as status:
            ...
            status.update(label=f"Output folder ready", state="complete")
        ...
        for i, frame_path in enumerate(frames):
            ...
            try:
                ...
                elif model_config["type"] == "qwen-portal":
                    scores = analyse_frame_qwen(frame_path, full_prompt, timeout)
